...
/*
  ...
  To compile (Unix):
    javac -classpath ../../target/evernote-api-*.jar YXoperates.java
 
  To run:
    java -classpath ../../target/evernote-api-*.jar YXoperates
...
public class YXoperates{
  ...
  private static final String AUTH_TOKEN = "S=s1:U=93ea3:E=1650ea710f0:C=15db6f5e2d0:P=1cd:A=en-devtoken:V=2:H=3b93ca53ca8fbbd244469ea69d080e5a";
  ...
  public static void main(String args[]) throws Exception {
	  YXoperates yx=new YXoperates(AUTH_TOKEN);
   ...
   */
  public YXoperates(String token) throws Exception {
    // Set up the UserStore client and check that we can speak to the server
    EvernoteAuth evernoteAuth = new EvernoteAuth(EvernoteService.SANDBOX, token);
    ClientFactory factory = new ClientFactory(evernoteAuth);
    userStore = factory.createUserStoreClient();

    boolean versionOk = userStore.checkVersion("Evernote YXoperates (Java)",
  ...
  private void createNote() throws Exception {
    ...
    Note note = new Note();
    note.setTitle("Test note from YXoperates.java");
  ...
  private void searchNotes() throws Exception {
    ...
    // In this example, we search for notes that have the term "YXoperates" in
    // the title.
    // This should return the sample note that we created in this demo app.
    String query = "intitle:YXoperates";
  ...
  private static Data readFileAsData(String fileName) throws Exception {
    String filePath = new File(YXoperates.class.getResource(
        YXoperates.class.getCanonicalName() + ".class").getPath()).getParent()
